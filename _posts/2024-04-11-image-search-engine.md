...
Our client had been analyzing their customer feedback, and one thing in particular came up many times.
...
They buy more expensive products and later find out that we stocked a similar but lower-priced alternative.
...
Here we implement the pre-trained VGG16 network. Instead of the final MaxPooling layer, we add a **Global Average Pooling Layer** at the end of the VGG16 architecture for the output to be a single vector of numeric information rather than many arrays.  We use "feature vector" to compare image similarity.

We pre-process our 300 base-set images and then pass them through the VGG16 network to extract their feature vectors.  We store these in an object to use when a search image is fed in.
...
We have coded this up for the "proof of concept".  In practice we would have the last section of the code (where we submit a search) isolated, and running from all of the saved objects that we need - we wouldn't include it in a single script like we have here.

we could store that object, rather than having to fit the Nearest Neighbours to our *feature_vector_store* each time a search is submitted.

When applying this in production, we would code up a script that easily adds or removes images from the feature store.  The products that are available in the client store would be changing all the time, so we'd want a nice easy way to add new feature vectors to the feature_vector_store object - and also potentially a way to remove search results coming back if that product was out of stock, or no longer part of the suite of products that were sold.

Most likely, in production, this would  return a list of file paths that the client's website could then pull forward as required - the matplotlib code is just for us to see it in action manually!
...
It would be beneficial to come up with a way to quantify the quality of the search results.  This could come from customer feedback, or click-through rates on the site.
...
Transfer Learning is an extremely powerful way for us to utilise pre-built, and pre-trained networks, and apply these to solve *our* specific Deep Learning based tasks.  It consists of taking features learned on one problem, and leveraging them on a new, similar problem!
...
For our Fruit Classification task, we will use a famous network known as **VGG16**.  This was designed back in 2014, but even by today's standards is a fairly heft network.  It was trained on the famous *ImageNet* dataset, with over a million images across one thousand different image classes. Everything from goldfish to cauliflowers to bottles of wine, to scuba divers!

<br>
![alt text](/img/posts/vgg16-architecture.png "VGG16 Architecture")

<br>
The VGG16 network won the 2014 ImageNet competition. It predicted more accurately than any other model on that set of images (although this has now been surpassed).
...
All the hard work has been done, we want to "transfer" those "learnings" to our own problem space.

<br>
#### Nuanced Application

When using Transfer Learning for image classification tasks, we often import the architecture up to the final Max Pooling layer, before flattening & the Dense Layers & Output Layer.  We use the frozen parameter values from the bottom of the network and then get instead of the final Max Pooling layer

With this approach, the final MaxPooling layer will be in the form of several pooled feature maps.  For our task here, however, we don't want that. We want a *single set* of numbers to represent these features and add in a **Global Average Pooling Layer** at the end of the VGG16 architecture meaning the output of the network will be a single array of numeric information rather than many arrays.
...
Here we create two useful functions, one for pre-processing images before entering the network, and the second for featurising the image, in other words passing the image through the VGG16 network and receiving the output, a single vector of 512 numeric values.
...
We now want to preprocess & feature all 300 images in our base set.  To do this we execute a loop and apply the two functions we created earlier.  For each image, we will append the filename and the feature vector to stores.  We then save these stores, for future use when a search is executed.
...
With the base-set featurised, we can now search using a new image from a customer!
...
Using the same helper functions, we apply the preprocessing & featurising logic to the search image - the output will be a vector containing 512 numeric values.
...
At this point, we have our search image  as a 512-length feature vector, and we need to compare that feature vector to the feature vectors of all our base images.

When that is done, we need to understand which of those base image feature vectors are most like the feature vector of our search image, and more specifically, we need to return the eight most closely matched.

To do this, we use the *NearestNeighbors* class from *scikit-learn*. We will apply the *Cosine Distance* metric to calculate the angle of difference between the feature vectors.

**Cosine Distance** essentially measures the angle between any two vectors, and it looks to see whether two vectors are pointing in a similar direction.  The more similar the direction the vectors are pointing, the smaller the angle between them in space, and the more different the direction the LARGER the angle between them in space. This angle gives us our cosine distance score.

By calculating this score between our search image vector and each of our base image vectors, we can return the images with the eight lowest cosine scores - and these will be our eight most similar images, at least in terms of the feature vector representation that comes from our VGG16 network!
...
```
<br>
The search image and search results are shown below:
...
<br>
Again, these have come out well - the features from VGG16 combined with Cosine Similarity have done a great job!
...
When applying this in production, we may code up a script that easily adds or removes images from the feature store.  The products that are available in the client store would be changing all the time, so we'd want a nice easy way to add new feature vectors to the feature_vector_store object - and also potentially a way to remove search results coming back if that product was out of stock, or no longer part of the suite of products that were sold.

Most likely, in production, this would return a list of filepaths that the client's website could then pull forward as required - the matplotlib code is just for us to see it in action manually!

This was tested only in one category, we would want to test on a broader array of categories - most likely having a saved network for each to avoid irrelevant predictions.

We only looked at Cosine Similarity here, it would be interesting to investigate other distance metrics.

It would be beneficial to come up with a way to quantify the quality of the search results.  This could come from customer feedback or via click-through rates on the site.
